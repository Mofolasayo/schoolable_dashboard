...
export async function createCompliancePolicy(
...
): Promise<{ success: boolean; data?: CompliancePolicy; error?: string }> {
  const token = await getAuthToken();
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: error.error || 'Failed to create policy',
      };
    }
...
export async function updateCompliancePolicy(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: error.error || 'Failed to update policy',
      };
    }
...
export async function getComplianceSubmissions(
  ...
  try {
    const response = await fetch(
      `${API_URL}/compliance/policies/${policyId}/submissions`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
export async function reviewComplianceSubmission(
  ...
  try {
    const response = await fetch(
      `${API_URL}/compliance/submissions/${submissionId}/review`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
